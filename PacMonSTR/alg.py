...
def run_pacmonstr_alg(prefix,suffix,motif,seq):
    q_seq = map(ord,seq.upper())
    q_seq = np.asarray(q_seq)
    p_s_t = [prefix.upper(),suffix.upper(),motif.upper()]
    number_of_tr, repeat_aligned, suffix_aligned, prefix_aligned = alignRegions(q_seq,p_s_t)
    repeat_score = None
    suffix_score = None
    prefix_score = None
    if number_of_tr != None:
        repeat_score = score_alignment(repeat_aligned[0],repeat_aligned[1])
        suffix_score = score_alignment(suffix_aligned[0],suffix_aligned[1])
        prefix_score = score_alignment(prefix_aligned[0],prefix_aligned[1])
    scores = (prefix_score,suffix_score,repeat_score)
    return (number_of_tr,scores)
...
if __name__ == '__main__': 
    ...
    for i,(q,p,s,t) in enumerate(zip(query,prefix,suffix,tr)):
        ...
        if number_of_tr == None:
            continue
        repeat_aligned_score = score_alignment(repeat_aligned[0],repeat_aligned[1])
        suffix_aligned_score = score_alignment(suffix_aligned[0],suffix_aligned[1])
        prefix_aligned_score = score_alignment(prefix_aligned[0],prefix_aligned[1])
        out = [i,q.id,t.id,t.seq,number_of_tr,
               ...
               repeat_aligned_score,suffix_aligned_score,prefix_aligned_score]
